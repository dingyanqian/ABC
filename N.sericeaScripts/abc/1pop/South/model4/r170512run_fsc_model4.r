# South population
# model4
# exponential growth after instantaneous size change model
...
arl_run <- "/home/biosys/Desktop/ABC/abc/observed/1pop_South/arl_run.ars" # path for arl_run.ars
ssdefs <- "/home/biosys/Desktop/ABC/abc/observed/1pop_South/ssdefs.txt" # path for ssdefs.txt
...
# growth rate
GRATE <- round(runif(n.total.sim, -0.05, 0), 5)

# time when population size changed
LOG10_TIME <- round(runif(n.total.sim, 0, 5), 5)
TIME <- 10^LOG10_TIME

# population size when bottleneck
NBOT <- NCUR*exp(GRATE*TIME)
...
prior.out <- data.frame(NCUR, NANC, GRATE, LOG10_TIME,
                        MUT_SHAPE, P_MEAN)
...
prior.ESTSSR <- data.frame(NCUR_HAP, GRATE, RSIZE, TIME,
